param(
    ...
    # Build a signed Wing development APK without Dart AOT compilation.
    [switch]$Development,
...
$keystore = Join-Path $signingRoot 'wing.p12'
...
if ($InitializeSigning) {
    ...
    $credential = [PSCredential]::new('wing', $password)
    # Windows DPAPI binds this to this user/computer. Not a portable backup.
    $credential | Export-Clixml -LiteralPath $credentialFile
    try {
        $env:WING_STORE_PASSWORD = $credential.GetNetworkCredential().Password
        & $keytool -genkeypair -keystore $keystore -storetype PKCS12 -alias wing -keyalg RSA -keysize 3072 -validity 10000 -dname 'CN=Wing' -storepass:env WING_STORE_PASSWORD -keypass:env WING_STORE_PASSWORD
    ...
    } finally { Remove-Item Env:WING_STORE_PASSWORD -ErrorAction SilentlyContinue }
}
...
# All Wing releases share one lock, even when called from separate snapshots.
$buildMutex = [Threading.Mutex]::new($false, 'Local\WingAndroidReleaseBuild')
...
try {
    ...
    if (!$ownsBuildMutex) { throw 'Another Wing release build is running. Let it finish before starting another.' }
    ...
    $env:WING_STORE_FILE = $keystore
    $env:WING_STORE_PASSWORD = $credential.GetNetworkCredential().Password
    $env:WING_KEY_ALIAS = $credential.UserName
    $env:WING_KEY_PASSWORD = $env:WING_STORE_PASSWORD
    $buildMode = 'release'
    $androidBuildArguments = @('--release', '--split-debug-info=build/wing-symbols', '--android-project-arg=shrink=false')
    if ($Development) {
        $buildMode = 'debug'
        $androidBuildArguments = @('--debug', '--android-project-arg=wingDevelopment=true')
        Write-Output 'Wing development APK: Dart JIT, assertions and debugging enabled; larger APK and slower runtime than release.'
    } elseif ($OptimizeAndroid) {
        $androidBuildArguments = @('--release', '--split-debug-info=build/wing-symbols')
        Write-Output 'Android code and resource optimization enabled.'
    } else {
        Write-Output 'Faster Wing build: Android shrinking disabled; Dart release compilation and Android Lint remain enabled.'
    }
    ...
    $expectedCertificate = (Get-Content (Join-Path $repository 'android\wing-release-certificate.sha256') -Raw).Trim()
    if ($signature -notcontains "Signer #1 certificate SHA-256 digest: $expectedCertificate") {
        throw 'APK was signed with a different key than the Wing release identity'
    }
    $signature | Write-Output
    $badging = & (Join-Path $buildTools 'aapt.exe') dump badging $apk
    if ($LASTEXITCODE -ne 0 -or !($badging -match "package: name='com.tarkilhk.wing'")) { throw 'Unexpected APK identity' }
    ...
    Write-Output "Verified Wing $buildMode APK: $apk"
} finally {
    foreach ($name in @('WING_STORE_FILE', 'WING_STORE_PASSWORD', 'WING_KEY_ALIAS', 'WING_KEY_PASSWORD')) {

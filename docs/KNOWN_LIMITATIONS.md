...
Connection recovery makes a short retry burst, then waits for app/chat focus, an Android network change or explicit Retry. Returning to the chat retries immediately. There is no continuous 30-second polling; if a server returns after that burst without a network-change event, reconnecting and background alerts wait for one of those triggers.

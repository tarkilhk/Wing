...
Tool model lists put the current or pending selection first. Tool setup provider lists put selected providers first and tint their cards, with a visible Selected label; Web names search and extraction selections separately. Keep the image/video provider controls and speech provider dropdown otherwise unchanged.

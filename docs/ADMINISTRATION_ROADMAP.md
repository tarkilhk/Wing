...
Administration now uses Profile and Health. Client version and Server version live in the global menu; only the server checks for upstream updates. Profile lifecycle remains reachable beside the Profile selector. Do not restart the historical first model-and-skills slice. Current limitations include positional memory IDs that prevent safe individual edits/deletes, whole-map MCP writes that prevent safe per-tool toggles and incomplete credential provenance. Pending sensitive forms can resume when Hermes supplies their exact open requests; other pending work still depends on its server contract.
...
The Server / Providers entry, shared-account links and root-inheritance wording
have been removed. Scope follows the owning resource rather than the existence
of a profile parameter.

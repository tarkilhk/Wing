...
| R07 | Original remote server: authenticated diagnostics, existing QA chat reopen/context, fresh nonce reply and durable reopen. | Moving to the emulator. Its real local-Hermes connection works; the original remote server presents a password form. Direct user login is being prepared. Signed 2.31.19 installed and cold-launched, but this is not a remote acceptance result. |
| R08 | Original remote server: attach a synthetic file whose content marker is absent from the prompt/filename; verify content-based reply and reopen. | Waiting for remote login on the emulator. A synthetic file and independent marker are prepared; no remote attachment was submitted during this attempt. The phone has been released and its original screen timeout restored. |

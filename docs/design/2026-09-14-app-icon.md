...
`ic_stat_hermes` is a 24 dp white alpha vector. The notification sink selects it by drawable name, and `res/raw/keep.xml` retains it during resource shrinking. API 33 adds the wing monochrome layer; API 26 keeps the regular portrait. Round and ordinary adaptive icons share a foreground.

...
The drawer groups Chats and Activity; Hermes instances and App settings; then Hermes administration, Hermes health and Hermes analytics. Select a connection and profile without changing another client's selection. Chats includes recent projects, pins, search and paginated history. Filters cover unread and automated chats; Activity can filter Running and Needs input and reports unreachable profiles.

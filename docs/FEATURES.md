...
The drawer groups Chats and Activity; Hermes instances and App settings; then Hermes administration, Hermes health and Hermes analytics. Select a connection and profile without changing another client's selection. Chats groups chats by Project, Updated, Status or Profile, with pins and three-chat previews. The **…** menu contains **Group by**, **Sort by** and **Show details**, each opening an anchored menu. Visible details include updated time, tokens, cost and profile. Changes apply immediately and are remembered per connection on this device. Separate Status, Profile and Project menus support multiple selections; automated chats have an independent visibility toggle; Activity can filter Running and Needs input and reports unreachable profiles.

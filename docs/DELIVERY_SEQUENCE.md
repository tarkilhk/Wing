...
The [implemented-feature QA sweep](QA_SWEEP_2026-09-13.md) is complete for every
concrete check available in the disposable emulator/local-Hermes environment.
The final 2.31.10 code passes 1,334 tests with four opt-in skips, clean analysis
and all eight native emulator scenarios with driver exit 0. The normal app was
restored on the emulator as 2.31.10-dev / 2195. No 2.31.10 phone deployment is
claimed.

Remaining acceptance needs external conditions, physical hardware,
authorization or an upstream contract. Broad K administration, background push,
...
| D14 | Live tool detail, timing and reasoning passed | Expandable live tools with args/results/server duration, revisioned server todos, live/historical reasoning | Real clarification details expanded, a completed live tool reported 211 s, and server-provided loop reasoning displayed as Thought | Todos and reasoning remain bounded by fields the server exposes |
...
| D20 | Implemented; final native scenario passed and live writes unauthorized | Project create/rename/appearance/delete and membership. The release adds explicit repository-root discovery through stock `projects.discover_repos`, with manual absolute-path fallback | The completed-code final native project scenario passed. Earlier isolated create/rename/cancel-delete/delete and live discovery also passed | A real create/edit write was not authorized and remains unverified |
...
| D24 | Goal and criteria controls passed live | Server goal/verification details, Pause/Resume/Unwait/Clear and criteria Add/Remove/Clear. See [session controls](SESSION_CONTROLS.md) | Local Hermes passed Pause/Resume, goal Clear, and criteria Add/Remove/Clear with exact retained/cleared counts. The earlier busy Resume partial failure remains recorded | Goal contracts and gates remain read-only. Unwait is conditional because installed `/goal wait` requires an integer PID and no safely owned wait barrier was available |

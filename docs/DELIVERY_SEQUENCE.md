...
| D01-D04 | Implemented; core Samsung QA passed | Durable drafts, server refresh on cached reopen, technical-provider groups, session YOLO; removed local model overrides. See [delivery notes](CONVERSATION_FOUNDATIONS.md) | Samsung verified draft restart, existing-chat reopen, provider grouping, per-chat reasoning and `/yolo` during a running response. Emulator clarification recovery, cross-client model/reasoning adoption and transparent-proxy lost-ack duplicate prevention passed on 2.31.9; see the [QA sweep](QA_SWEEP_2026-09-13.md) | Missing-file recovery remains automated evidence |
...
| D06 | Clarification live QA passed; approval variants pending | Existing clarification handling retained; server-advertised approval scopes and request targeting | Single-question force-stop recovery and a two-question BLUE/DOG response passed against local Hermes | Advertised approval-scope variants remain unverified |
...
| D14 | Live tool detail and timing passed | Expandable live tools with args/results/server duration, revisioned server todos, live/historical reasoning | Real clarification details expanded and a completed live tool reported 211 s | Todos and reasoning remain bounded by fields the server exposes |
...
| D24 | Goal Pause, Resume and Clear passed live | Server goal/verification details, Pause/Resume/Unwait/Clear and criteria Add/Remove/Clear. See [session controls](SESSION_CONTROLS.md) | Local Hermes passed idle Pause/Resume through Active to Done 1/20 and confirmed Clear left no active goal. The earlier busy Resume partial failure remains recorded | Goal contracts and gates remain read-only; other supported goal actions remain open |
| D25 | Live process controls passed; loop scope limited upstream | Session heartbeat/loop state, supported controls and targeted process Stop/Dismiss | Live process details showed command, PID and output; Stop produced exit code -15 and Dismiss emptied the panel. See the [QA sweep](QA_SWEEP_2026-09-13.md) | Dismiss remains transient client view state. QA031 found official Hermes loop commands write unscoped state while Android and Desktop read profile-scoped state. No Android parser or backend change is claimed; default-profile loop control remains untested |
...
| D30 | Versions and live read-only update status passed | Installed Android identity, this fork's changelog and release page; backend update eligibility, confirmation, progress and correlated per-host outcomes | Version/update checks are included in the 2.30.1 baseline; the 2.31.9 live probe parsed eligibility and status without starting an update | TUI restart is deferred without an existing remote API. Release-link QA remains; no actual backend update was performed |

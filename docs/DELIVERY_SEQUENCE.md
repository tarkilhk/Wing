...
| D08 Make current notifications useful | R16, R17, S12, M09; local portion of M01 | Local completion/input notices, simple controls and notification taps work together. Tapping loads the right server session. The app describes local-delivery limits; Firebase D26/D27 is dropped. |
...
## 7. Notification scope correction — 2026-09-14

D26/D27 Firebase delivery is dropped, not deferred work. Existing local alerts and authenticated tap routing remain selected. Sparse alerts are under investigation; permission is healthy, but current live-event coverage is limited to loaded chats. See [notification findings](ADMINISTRATION_ROADMAP.md#notification-findings). No Hermes backend changes are authorized.

## 8. Complete connection maintenance and the small admin screen

The initial small administration flow is delivered. Broader administration was selected on 2026-09-14; follow the ranked slices in [the administration roadmap](ADMINISTRATION_ROADMAP.md), starting with default models, skills and tools. Reuse the existing screen and server APIs.
...
| D29 Edit the selected profile | B14, B15; initial K seed | Description/SOUL operations use explicit server scope. Client-local profile navigation and centrally shared server settings remain distinct. Subsequent provider/tool/plugin work follows the administration roadmap. |
...
The remaining real-backend QA has a finite case list in
[remaining acceptance](QA_REMAINING_ACCEPTANCE_2026-09-14.md). The normal signed
app on the emulator now passes actual Hermes PDF/image/SVG zoom, PDF paging,
WAV/MP3/MP4/WebM native playback controls, paused-position restoration,
Markdown-linked media and invalid-format recovery. This closes the earlier
media/zoom gap without requiring the user's phone.

Real manual approval cards now pass Deny and Allow once. Vault acceptance is
blocked by the stock browser failing before a request is emitted. The original
remote deployment's attachment-content and saved-chat reopen cases remain
separate from passing local-backend checks. Feature releases in other tasks do
not change these acceptance requirements or authorize backend modifications.

### Previous baseline, 2026-09-13
...
| D26-D27 / M01, R16, R17, S12, M09 | Superseded 2026-09-14: Firebase delivery is dropped. Existing local alerts remain supported and missing-alert coverage is under investigation. |
...
| D06 | Clarification and manual approval live QA passed | Existing clarification handling retained; server-advertised approval scopes and request targeting | Single-question force-stop recovery and a two-question BLUE/DOG response passed against local Hermes. Real manual approval cards accepted Deny and Allow once against an owned empty repository, and both results persisted | Only the server-advertised Deny and Allow once choices were tested |
| D07 | Live sudo and QA-034 secret Cancel/expiry passed; cold recovery deferred | Dedicated sudo/secret/vault forms handle existing live requests and expiry events. Same-runtime reconnect preserves received requests; credentials remain unsaved. The 2.31 release improves the older-history jump label | A real sudo request cancelled with an empty response and persisted its tool result. Default `/gif-search` produced a real `TENOR_API_KEY` request. Corrected Cancel and the untouched stock 300 s expiry both completed; all seven live-backend scenarios passed with driver exit 0 | Local vault acceptance is blocked by the stock browser failing before a request is emitted. No external vault manager is configured. Unmodified Hermes exposes no verified pending-sensitive snapshot; see [limits](SENSITIVE_REQUEST_RECOVERY.md) |
| D08 | Local posting and chat routing passed; Firebase dropped | Independent completion/attention switches, optional chat titles, permission/test alert and existing notification routing; 2.27.1 adds startup retry and stable chat alert IDs | Samsung verified the local test alert plus warm and process-cold routing for a real chat, including retained pending camera intake; see the [QA sweep](QA_SWEEP_2026-09-13.md) | Firebase delivery D26/D27 is dropped; native posting does not guarantee delivery while the process is absent |
| D09 | Samsung queues, Steer and lost-ack recovery passed | Desktop-style text/file queues with filename review and removal; next-draft attachment preparation survives active responses and picker reconnection | Samsung 2.31.6 verified ordered drain and acknowledged Steer. Local Hermes proved attachment contents. The 2.31.9 transparent-proxy test dropped a successful acknowledgement, then reconnect proved exactly one server row and one matching submission while the uncertain queue item remained for explicit removal. See the [QA sweep](QA_SWEEP_2026-09-13.md) | The app deliberately does not remove an uncertain item automatically. Remote QA018 now proves file contents through read_file and durable reopen; automatic @file expansion still has the backend workspace restriction, matching Desktop |
...
| D12 | Common content implemented; real-backend native zoom passed | Wide tables, selectable streamed/nested fenced code and copy/wrap controls; on-demand Mermaid/SVG previews with zoom and source access. See [diagram notes](DIAGRAM_PREVIEWS.md) | Existing content tests remain recorded; real server PDF and SVG downloads now pass native pinch-out/in and PDF paging. See [acceptance](QA_REMAINING_ACCEPTANCE_2026-09-14.md) | Other diagram formats retain source fallback |
| D13 | Common media and real-backend playback passed | Zoomable images, hosted web page previews and downloaded audio/video playback with native controls. Direct Markdown file links use the existing output viewer. See [opening files](OPENING_OUTPUT_FILES.md) | Real PNG zoom and WAV/MP3/MP4/WebM Play/Pause/Seek passed; decoded video, paused-position restoration and linked-media playback were verified on the emulator | Codec availability remains device-dependent; invalid media offers a verified return/open/save-share path |
...
| D17 | Selected previews and real-backend media acceptance passed | Authenticated image/SVG/text and Markdown/source, hosted web pages, downloaded self-contained HTML, binary save/share, external media handoff, in-app audio/video and PDF pages with zoom; see [opening files](OPENING_OUTPUT_FILES.md), [PDF reading](PDF_READING.md) and [HTML scope](MARKDOWN_AND_WEB_PREVIEWS.md) | Existing formatted Markdown checks plus normal-app real-backend PDF/image/SVG zoom, native media controls and invalid-format recovery pass. See [acceptance](QA_REMAINING_ACCEPTANCE_2026-09-14.md) | Two older generated paths return HTTP 404. HTML remains limited to 1 MiB and self-contained content; no physical-speaker claim |
...
| D26-D27 | Firebase delivery dropped | Existing local alerts and tap routing remain available | Native posting checks do not establish coverage of real events; missing alerts remain under investigation | No compatible existing Hermes sender; no backend changes authorized |
...
Q05 discoverability, B03 OAuth, B16-B20 bots, the separate administration candidates, section 11, M10 offline history and M11 local ranking remain excluded or deferred exactly as recorded in the product plan. M07, M12 and M13 remain future possibilities. M05 adds no separate work. M08 and S14 add no independent client-state feature.
